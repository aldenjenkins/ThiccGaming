...
class PollForm(forms.ModelForm):
    ...
    answers = forms.CharField(label=_("Answers. Write each answer on a new line. Minimum of 2:"), required=False, min_length=2, widget=forms.Textarea,
    )
    choice_count = forms.IntegerField(label=_("The number of answers the user can choose from the given answers in a single vote. (Recommended: 1):"), help_text=(_("Test")), required=False, min_value=1,
        error_messages={'min_value': _("Number of choices must be positive.")},
    )
    days = forms.IntegerField(label=_("Number of days for this poll to run. Leave empty for never ending poll:"), required=False, min_value=1, max_value=999, validators=[MaxValueValidator(999)]
    )

    class Meta:
        model = Poll
        fields = ['question', 'answers', 'choice_count','days']
    ...
    def clean(self):
        ...
        a = data.get('question',None)
        b = data.get('answers', None)
        c = data.get('choice_count', None)
        d = data.get('days', None)

        if (not a or not b or not c or not d) and (a or b or c or d):
            ...
            # if len(answers) != self.cleaned_data['choice_count']:
            #     raise forms.ValidationError(_("The amount of answers doesnt match the choice count!"))
            ...
            if len(answers) < self.cleaned_data['choice_count']:
                raise forms.ValidationError(_("You cannot allow more choices than there are possible answers."))

            if is_max_length > should_max_length:
                raise forms.ValidationError(_("One of the answers is too long. 200 character max."))
            else:
                return data
