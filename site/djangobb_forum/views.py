...
from thicc.apps.bans import models as bans
...
def add_topic(request, forum_id):
    ...
    if contest and id != '':
        ...
        ban = False
    post_form_kwargs = {"forum": forum, "user": request.user, "ip": ip, 'contest': contest}
    ...
    if contest:
        messages.warning(request, "Please complete the template below to submit your ban appeal to the forums.\n\
        ...
        if request.method == 'POST':
            if form.is_valid():
                try:
                    ...
                    print ("I/O error(%s): %s"%(errno_strerror[0], errno_strerror[1]))
